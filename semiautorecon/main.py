...
async def get_semaphore(semiautorecon):
	...
	while True:
		...
		if semaphore.locked():
			if semaphore != semiautorecon.port_scan_semaphore: # This will be true unless user sets max_scans == max_port_scans
				...
				if not semiautorecon.pending_targets and (config['max_port_scans'] - port_scan_task_count) >= 1: # If no more targets, and we have room, use port scan semaphore.
					if semiautorecon.port_scan_semaphore.locked():
						await asyncio.sleep(1)
						continue
					semaphore = semiautorecon.port_scan_semaphore
					break
				else: # Do some math to see if we can use the port scan semaphore.
					if (config['max_port_scans'] - (port_scan_task_count + (len(semiautorecon.pending_targets) * config['port_scan_plugin_count']))) >= 1:
						if semiautorecon.port_scan_semaphore.locked():
							await asyncio.sleep(1)
							continue
						semaphore = semiautorecon.port_scan_semaphore
...
async def port_scan(plugin, target):
	...
	async with target.semiautorecon.port_scan_semaphore:
...
async def service_scan(plugin, service):
	...
	if not config['force_services']:
		semaphore = await get_semaphore(service.target.semiautorecon)
	...
	while plugin_pending:
		...
		if plugin.max_global_instances and plugin.max_global_instances > 0:
			async with service.target.semiautorecon.lock:
				# Count currently running plugin instances.
				for target in service.target.semiautorecon.scanning_targets:
...
async def generate_report(plugin, targets):
	...
	if not config['force_services']:
		semaphore = await get_semaphore(semiautorecon)
...
async def scan_target(target):
	...
	if config['force_services']:
		...
		if services:
			...
			heartbeat.cancel()
			semiautorecon.errors = True
			return
	else:
		for plugin in target.semiautorecon.plugin_types['port']:
			...
			if config['port_scans'] and plugin.slug in config['port_scans']:
				...
				matching_tags = False
				for tag_group in target.semiautorecon.tags:
					if set(tag_group).issubset(plugin_tag_set):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in target.semiautorecon.excluded_tags:
					if set(tag_group).issubset(plugin_tag_set):
						excluded_tags = True
						break
	...
	async with semiautorecon.lock:
		semiautorecon.scanning_targets.append(target)
	...
	while pending:
		...
		for service in services:
			...
			nmap_extra = target.semiautorecon.args.nmap
			if target.semiautorecon.args.nmap_append:
				nmap_extra += ' ' + target.semiautorecon.args.nmap_append
			...
			for plugin in target.semiautorecon.plugin_types['service']:
			...
			if not service_match:
				...
				if service.name not in ['status', 'tcpwrapped', 'unknown'] and service.full_tag() not in target.semiautorecon.missing_services:
					target.semiautorecon.missing_services.append(service.full_tag())

	for plugin in target.semiautorecon.plugin_types['report']:
	...
	async with semiautorecon.lock:
		semiautorecon.completed_targets.append(target)
		semiautorecon.scanning_targets.remove(target)
...
async def run():
			...
			if not plugin_file.startswith('_') and plugin_file.endswith('.py'):
				...
				try:
					...
					for (_, c) in clsmembers:
						if c.__module__ in ['semiautorecon.plugins', 'semiautorecon.targets']:
							continue
						...
						if issubclass(c, PortScan) or issubclass(c, ServiceScan) or issubclass(c, Report):
							semiautorecon.register(c(), filename)
						else:
	...
	for plugin in semiautorecon.plugins.values():
		if plugin.slug in semiautorecon.taglist:
			unknown_help()
	...
	if len(semiautorecon.plugin_types['port']) == 0:
		unknown_help()
		fail('Error: There are no valid PortScan plugins in the plugins directory "' + config['plugins_dir'] + '".')

	# Sort plugins by priority.
	semiautorecon.plugin_types['port'].sort(key=lambda x: x.priority)
	semiautorecon.plugin_types['service'].sort(key=lambda x: x.priority)
	semiautorecon.plugin_types['report'].sort(key=lambda x: x.priority)
						...
						if 'pattern' in pattern:
							try:
								...
								if 'description' in pattern:
									semiautorecon.patterns.append(Pattern(compiled, description=pattern['description']))
								else:
									semiautorecon.patterns.append(Pattern(compiled))
							except re.error:
								unknown_help()
		...
		if key == 'global' and isinstance(val, dict): # Process global plugin options.
			for gkey, gval in config_toml['global'].items():
				if isinstance(gval, bool):
					for action in semiautorecon.argparse._actions:
						if action.dest == 'global.' + slugify(gkey).replace('-', '_'):
							if action.const is True:
								action.__setattr__('default', gval)
							break
				else:
					if semiautorecon.argparse.get_default('global.' + slugify(gkey).replace('-', '_')):
						semiautorecon.argparse.set_defaults(**{'global.' + slugify(gkey).replace('-', '_'): gval})
		elif isinstance(val, dict): # Process potential plugin arguments.
			for pkey, pval in config_toml[key].items():
				if semiautorecon.argparse.get_default(slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_')):
					semiautorecon.argparse.set_defaults(**{slugify(key).replace('-', '_') + '.' + slugify(pkey).replace('-', '_'): pval})
		else: # Process potential other options.
			key = key.replace('-', '_')
			if key in configurable_keys:
				other_options.append(key)
				config[key] = val
				semiautorecon.argparse.set_defaults(**{key: val})
	...
	if config['max_plugin_target_instances']:
		for plugin_instance in config['max_plugin_target_instances']:
			...
			if len(plugin_instance) == 2:
				if plugin_instance[0] not in semiautorecon.plugins:
	...
	if not errors:
		if config['force_services']:
			semiautorecon.service_scan_semaphore = asyncio.Semaphore(config['max_scans'])
		else:
			semiautorecon.port_scan_semaphore = asyncio.Semaphore(config['max_port_scans'])
			...
			if config['max_scans'] == config['max_port_scans']:
				semiautorecon.service_scan_semaphore = semiautorecon.port_scan_semaphore
			else:
				semiautorecon.service_scan_semaphore = asyncio.Semaphore(config['max_scans'] - config['max_port_scans'])
	...
	[semiautorecon.tags.append(t) for t in tags if t not in semiautorecon.tags]
	...
	if args.exclude_tags != '':
		...
		# Remove duplicate lists from list.
		[semiautorecon.excluded_tags.append(t) for t in excluded_tags if t not in semiautorecon.excluded_tags]
	...
	for target in raw_targets:
		try:
			...
			found = False
			for t in semiautorecon.pending_targets:
				if t.address == ip_str:
					found = True
					break
			...
			try:
				...
				try:
					...
					semiautorecon.pending_targets.append(Target(target, ip, 'IPv4', 'hostname', semiautorecon))
					...
					try:
						...
						semiautorecon.pending_targets.append(Target(target, ip, 'IPv6', 'hostname', semiautorecon))
					except socket.gaierror:
						error(target + ' does not appear to be a valid IP address, IP range, or resolvable hostname.')
						errors = True

	if len(semiautorecon.pending_targets) == 0:
		error('You must specify at least one target to scan!')
		errors = True

	if config['single_target'] and len(semiautorecon.pending_targets) != 1:
		error('You cannot provide more than one target when scanning in single-target mode.')
		errors = True

	if not args.disable_sanity_checks and len(semiautorecon.pending_targets) > 256:
		error('A total of ' + str(len(semiautorecon.pending_targets)) + ' targets would be scanned. If this is correct, re-run with the --disable-sanity-checks option to suppress this check.')
		errors = True

	if not config['force_services']:
		port_scan_plugin_count = 0
		for plugin in semiautorecon.plugin_types['port']:
			if config['port_scans'] and plugin.slug in config['port_scans']:
				matching_tags = True
				excluded_tags = False
			else:
				matching_tags = False
				for tag_group in semiautorecon.tags:
					if set(tag_group).issubset(set(plugin.tags)):
						matching_tags = True
						break

				excluded_tags = False
				for tag_group in semiautorecon.excluded_tags:
					if set(tag_group).issubset(set(plugin.tags)):
						excluded_tags = True
						break
	...
	pending = []
	i = 0
	while semiautorecon.pending_targets:
		pending.append(asyncio.create_task(scan_target(semiautorecon.pending_targets.pop(0))))
		i+=1
	...
	while pending:
		...
		# If something failed in scan_target, semiautorecon.errors will be true.
		if semiautorecon.errors:
		...
		for task in done:
			if semiautorecon.pending_targets:
				pending.add(asyncio.create_task(scan_target(semiautorecon.pending_targets.pop(0))))
			if task in pending:
				pending.remove(task)

		port_scan_task_count = 0
		for targ in semiautorecon.scanning_targets:
	...
	if len(semiautorecon.completed_targets) > 1:
		for plugin in semiautorecon.plugin_types['report']:
			...
			if matching_tags and not excluded_tags:
				pending.add(asyncio.create_task(generate_report(plugin, semiautorecon.completed_targets)))
