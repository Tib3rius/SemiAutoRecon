import asyncio, inspect, os, re, sys, termios
from typing import final
from semiautorecon.config import config
from semiautorecon.io import slugify, info, warn, error, fail, CommandStreamReader
...
class Plugin(object):
	...
	def __init__(self):
		...
		self.priority = 1
		self.patterns = []
		self.semiautorecon = None
		self.disabled = False

	@final
	def add_option(self, name, default=None, help=None):
		self.semiautorecon.add_argument(self, name, metavar='VALUE', default=default, help=help)

	@final
	def add_constant_option(self, name, const, default=None, help=None):
		self.semiautorecon.add_argument(self, name, action='store_const', const=const, default=default, help=help)

	@final
	def add_true_option(self, name, help=None):
		self.semiautorecon.add_argument(self, name, action='store_true', help=help)

	@final
	def add_false_option(self, name, help=None):
		self.semiautorecon.add_argument(self, name, action='store_false', help=help)

	@final
	def add_list_option(self, name, default=None, help=None):
		self.semiautorecon.add_argument(self, name, nargs='+', metavar='VALUE', default=default, help=help)
	...
	def add_choice_option(self, name, choices, default=None, help=None):
		...
		self.semiautorecon.add_argument(self, name, choices=choices, default=default, help=help)
	...
	def get_option(self, name):
		...
		if name in vars(self.semiautorecon.args):
			return vars(self.semiautorecon.args)[name]
		else:
			return None
	...
	def get_global_option(self, name, default=None):
		...
		if name in vars(self.semiautorecon.args):
			if vars(self.semiautorecon.args)[name] is None:
				if default:
					return default
				else:
					return None
	...
	async def execute(self, cmd, target, tag, patterns=None, outfile=None, errfile=None):
		...
		while True:
			...
			if confirm.strip() == 'EXECUTE':
				...
				cout = CommandStreamReader(process.stdout, target, tag, patterns=combined_patterns, outfile='/dev/null')
				cerr = CommandStreamReader(process.stderr, target, tag, patterns=combined_patterns, outfile='/dev/null')
