...
from semiautorecon.io import e, info, warn, error

class Target:

	def __init__(self, address, ip, ipversion, type, semiautorecon):
		self.address = address
		self.ip = ip
		self.ipversion = ipversion
		self.type = type
		self.semiautorecon = semiautorecon
		self.basedir = ''
		self.reportdir = ''
		self.scandir = ''
	...
	def extract_service(self, line, regex=None):
		return self.semiautorecon.extract_service(line, regex)

	async def extract_services(self, stream, regex=None):
		return await self.semiautorecon.extract_services(stream, regex)
	...
	async def execute(self, cmd, blocking=True, outfile=None, errfile=None, future_outfile=None):
		...
		info('Port scan {bblue}' + plugin.name + ' {green}(' + tag + '){rst} will run the following command against {byellow}' + address + '{rst}: ' + cmd, verbosity=2)
		...
		process, stdout, stderr = await target.semiautorecon.execute(cmd, target, tag, patterns=plugin.patterns, outfile=outfile, errfile=errfile)
...
class Service:
	...
	async def execute(self, cmd, blocking=True, outfile=None, errfile=None, future_outfile=None):
		...
		info('Service scan {bblue}' + plugin.name + ' {green}(' + tag + '){rst} will run the following command against {byellow}' + address + '{rst}: ' + cmd, verbosity=2)
